...
{
    public class TableStatisticStream
    {
        public CloudTable Table { get; }
        public string Filter { get; set; }
        public string Partition { get; set; }
        public int Parallelism { get; set; } = Environment.ProcessorCount;
        public ITableStatisticConsumer Consumer { get; }
        public BlockingCollection<(long ms, int num)> Statistics { get; }

        public TableStatisticStream(string source, string sourceKey, ITableStatisticConsumer consumer)
        {
            ...
            async Task GenerateDataForQuery(TableQuery query)
            {
                ...
                try
                {
                    do
                    {
                        ...
                        {
                            await Consumer.TakeAsync(segment.Results.Count);
        ...
        private async Task StartStatisticsDisplay()
        {
            ...
            void ReportStat()
            {
                ...
                Console.Write($"\rProcessed {totalNum / totalSec}/s, total: {overallTotal}       ");
